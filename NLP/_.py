...
def get_dataloaders():
    ...
    # 映射数据、处理数据
    def _trans_data(tags_label, max_seq_length=256, is_test=False):
        if is_test:
            tags = tags_label
        else:
    ...
    def _get_dataloader(dataset, batch_size=1, is_train=False, is_GPU=False):
        ...
        dataset = TagsDataset(list(map(_trans_data, dataset)))
    ...
    train_loader = _get_dataloader(train_ds, is_train=True, batch_size=BATCH_SIZE)
    dev_loader = _get_dataloader(dev_ds, batch_size=BATCH_SIZE)
    test_loader = _get_dataloader(test_ds,  batch_size=BATCH_SIZE)
...
def predict(model, data, tokenizer, label_map, batch_size=1):
    examples = []
    for text in data:
        input_ids, segment_ids = _tra(text, tokenizer, label_list=label_map.values(),  max_seq_length=128, is_test=True)
